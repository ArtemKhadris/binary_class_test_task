...
Video should be a usual .mp4 video that you have used for prediction, ```prediction.json``` should look like ```[[16, 18], [34, 36], [71, 95]]``` and a ```real.js``` should look like ```[[14, 19], [34, 38], [73, 95]]```.
